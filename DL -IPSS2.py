...
ax2.set_ylabel("Fuel Consumption Weight (lbf)")
ax2.set_title("Fuel Consumption Weight over Time")
ax2.grid(True)
...
def save_data(x_time, y_time):
    ...
    with open('time_data2.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Elapsed Time (s)', 'Fuel Consumption Weight (lbf)'])
