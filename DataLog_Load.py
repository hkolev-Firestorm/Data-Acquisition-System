...
# Save x and y data into .csv file - Copy 'sample_data.csv' and/or 'time_data.csv'
def save_data(x_data, y_data, x_time, y_time):
    with open('sample_data.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Sample #', 'Thrust (lbf)'])
        writer.writerows(zip(x_data, y_data))

    with open('time_data.csv', 'w', newline='') as file:
        writer = csv.writer(file)
...
try:
    while running:
        ...
        if keyboard.is_pressed('space'):  # Check if spacebar is pressed to stop
    ...
    save_data(x_data, y_data, x_time, y_time)
